...
if __name__=='__main__':
    ...
    '''Using this script:
    This is a script meant for call from a command line (or similar). It should be called as
    
        <python executable, typically just 'python'> xrb_root_f.py <inclination i> <outer radius filling factor f_r>
    
    and will construct light curves for these two values for every combination of T2, T_Disc and R_in
    possible (provided within script). This makes parallelisation very easy, as you simply call multiple versions of
    this script without requiring it to be changed between calls.    
    '''
    ...
    Ms = 1.9891e30  # solar mass in kg
    Rs = 6.96342e8  # solar radius in m
    Ts = 5778.0     # solar effective surface Temperature in K
    G = const.G     # gravitational constant in (m^3)/(kg s^2)  

    ## Specify
    # period = 0.32301405 ### A06
    # period = 0.432060249 ### GRS1124
    # period = 26.79377/24 ### 4U1543
    # period = 2.54451 ### BW Cir
    phaseoffset = 0
    DiscrimOnOff = 'OFF'
    DiscOnOff = 'ON'
    HsOnOff   = 'OFF'
    ...
    ### A06
    T_srange = np.arange(3750, 5251, 250)         # 06
    Trange = np.arange(1000, 4501, 250)           # 15
    R_inner_range = np.arange(1000, 10001, 1000)   # 10
    ...
    ### check the safety pin and exit if it isn't set to True
    if Bastelfreigabe is not True:
        logging.warning('Safety Pin was not released. Exiting!')
        raise
        
    try:
        
        Inkl = int(sys.argv[-1].split()[-2])
        f_r = float(sys.argv[-1].split()[-1])
        # logging.info(sys.argv)
        # Inkl = int(sys.argv[-2])
        # f_r = float(sys.argv[-1])
        
    except IndexError: ### only one argument given
        logging.info(f'Only one argument passed. Please provide "Inkl" "f_r" to be computed (passed {sys.argv}). Exiting.')
        raise
        
    except ValueError: ### argument cannot be converted properly
        logging.error(f'Please pass either one or two float as arguments, not {sys.argv[-1].split()[-2]} (type {type(sys.argv[-1].split()[-2])}). Exiting.')
        raise
        
    except: ### anything else
        raise RuntimeError(f'Unknown error occured with input {sys.argv[-1]} as input. Exiting.')

    ### make sanity checks on the arguments before attempting to parse them on:
    assert 10 <= Inkl <= 90, f'Inklination value outside intended range [10, 90] (passed: {Inkl})'
    assert 0. <= f_r <= 1.0, f'Outer radius ratio outside sensible range [0, 1] (passed: {f_r})'

    logging.info(f"Starting process with {Inkl} and {f_r}. Let's see, how long it survives...")

    ### this makes a list with all possible combinations of values contained in R_inner_range and T_srange, with Inkl and f_r
    combination_list = list(product([Inkl], [f_r], R_inner_range, T_srange))

    ### this is merely for debugging purposes, but you are free to leave it in.
    logging.info(f'Attempting the following combinations: {combination_list}; should ideally spawn {len(combination_list)} processes')

    ### call multiple parallel processes (72 at max) to compute the grid
    with Pool(processes=min([len(combination_list), 72])) as pool:
        pool.map(building_block, combination_list, 1)

    logging.info(f'Done for f_r={f_r} and Inkl={Inkl}. Exiting the script successfully. Good job :)')
